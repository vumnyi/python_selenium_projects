...
def test_add_item(browser, admin_autorization):
    ...
    browser.find_element_by_xpath(Admin.AdminButtonsEditItem.save_button).click()
    browser.find_element_by_xpath(Admin.AdminFilter.product_name_input).send_keys(PRODUCT_NAME)
    browser.find_element_by_xpath(Admin.AdminFilter.model_name_input).send_keys(MODEL)
    browser.find_element_by_xpath(Admin.AdminFilter.filter_button).click()
    parse_product_names = browser.find_elements_by_xpath(Admin.ProductList.product_names)
    try:
...
def test_edit_item(browser, admin_autorization):
    ...
    browser.find_element_by_xpath(Admin.AdminFilter.product_name_input).send_keys(PRODUCT_NAME)
    browser.find_element_by_xpath(Admin.AdminFilter.filter_button).click()
    browser.find_element_by_xpath(Admin.AdminButtonsEditItem.edit_button).click()
    browser.find_element_by_xpath(Admin.AddProduct.topics('Data')).click()
    model_old = browser.find_element_by_xpath(Admin.AddProduct.Data.model_input)
    model_old.clear()
    model_old.send_keys(NEW_MODEL)
    browser.find_element_by_xpath(Admin.AdminButtonsEditItem.save_button).click()
    parse_product_models = browser.find_elements_by_xpath(Admin.ProductList.product_models)
    try:
...
def test_delete_item(browser, admin_autorization):
    ...
    WebDriverWait(browser, 5).until(
        EC.visibility_of_element_located((By.XPATH, Product.Alert.success)))
    parse_product_models = browser.find_elements_by_xpath(Admin.ProductList.product_models)
